 ...
 */
public class ByteBuddyDecoratorAwareProxyGenerator {
    ...
    private final Map<Object, RuntimeDecoratorChain> activeChains =
     ...
     * @return the outermost decorated reference
     * @throws IllegalStateException if decorator creation or injection fails
     */
    public Object createDecoratorChain(
        ...
        // If no decorators, return the target unchanged.
        ...
        RuntimeDecoratorChain chain = new RuntimeDecoratorChain(decoratorInstances);
        activeChains.put(exposedOutermost, chain);
        if (rawOutermost != exposedOutermost) {
            activeChains.put(rawOutermost, chain);
        }
        return exposedOutermost;
    }
    ...
    public void destroyDecoratorChain(Object outermostInstance) {
        ...
        List<Object> decorators = chain.getDecoratorInstances();
        for (int i = decorators.size() - 1; i >= 0; i--) {
            Object decoratorInstance = decorators.get(i);
            try {
    ...
    private static final class RuntimeDecoratorChain {
        private final List<Object> decoratorInstances;

        private RuntimeDecoratorChain(List<Object> decoratorInstances) {
            this.decoratorInstances = Collections.unmodifiableList(new ArrayList<>(decoratorInstances));
        }

        private List<Object> getDecoratorInstances() {
            return decoratorInstances;
        }

        private boolean isEmpty() {
            return decoratorInstances.isEmpty();
        }
    }
